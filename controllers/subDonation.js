...
export const createSubdonation = async (req, res) => {
  try {
    ...
    const campaign = await DonationCampaign.findById(campaign_id);
    if (!campaign) {
      return res.status(404).json({ error: 'Donation campaign not found' });
    }
    ...
    const subdonation = new Subdonation({
      name,
      featured_image: path,
      amount,
      description,
...
export const updateSubdonation = async (req, res) => {
  try {
    ...
    const subdonation = await Subdonation.findById(id);
    if (!subdonation) {
      return res.status(404).json({ error: 'Subdonation not found' });
    }
